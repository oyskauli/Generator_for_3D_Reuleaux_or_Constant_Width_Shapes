...
Generate a point cloud of points on the surface a random reuleaux/constant width shape. The sphere is the most commonly known, but more complex shapes also exist. As an example here is an animation from sphere to more complex shape(s), all with same width:
